...
// Central brand name for the whole client. Overridable at runtime through
// window.RUNTIME_ENV.REACT_APP_APPNAME (generated by entrypoint.sh in Docker)
// or at build time through VITE_APPNAME, so self-hosted deployments can
// rebrand without touching the source. Falls back to "Signara".
export const APP_NAME =
  getEnv()?.REACT_APP_APPNAME ||
...
export async function getBase64FromUrl(url, autosign) {
  ...
  if (!data.ok) {
    throw new Error(
      `Failed to fetch document (status ${data.status}). Ask the sender to re-share the document.`
    );
  }
...
export const embedDocId = async (pdfOriginalWH, pdfDoc, documentId) => {
  ...
  // Served locally from /fonts so the signing flow works on self-hosted
  // deployments even without internet access to external CDNs.
...
export const convertPdfArrayBuffer = async (url) => {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Failed to download the document (status ${response.status}): ${url}`
      );
      return "Error";
    }
...
export const mailTemplate = (param) => {
  const appName = APP_NAME;
  // hostUrl is the browser origin (window.location.origin) passed by callers
  // so the logo resolves within the same deployment instead of an external CDN.
  const logoBase = param.hostUrl || window.location.origin;
  const logo = `<div style='padding:10px'><img src='${logoBase}/logo.png' height='50' /></div>`;
